...
let arr3 = ['Apple', 
            {name: 'John'},
            true,
            function() {console.log('Hi World!')}, // висячая запятая **
        ]
...
// Очередь сообщений, которые надо показать, например.
// 
// Существует и другой вариант применения для массивов - 
// структура данных, называемая СТЕК.
// 
...
// Как колода карт, новые карты кладутся наверх, берутся оттуда же
// 
// PUSH  POP
// ↓↓↓↓  ↑↑↑↑
// //////////
...
// 
// 
// Массивы в JS могут работать и как очередь, и как стек. Можно добавлять/удалять элементы как в начало,
// так и в конец массива.
// В комп науках структура данных, делающая это возможным, называется двусторонняя очередь.
// 
...
console.log(colors.pop()); // удаляем 'orange' и выводим его

console.log(colors); // red, blue

// И pop() и at() возвращают последний элемент массива, но pop() также изменяет массив, удаляя его.
// 
...
console.log(colors.shift()); // удаляем 'red' и выводим его
...
color.push('blue', 'black'); // добавляем в конец
color.unshift('pink', 'brown'); // добавляем в начало

console.log(color); // pink, brown, red, blue, black


// Внутреннее устройство массива:
// 
// Массив - особый подвид объектов. Квадратные скобки, используемые для того, чтобы получить доступ к свойству
// arr[0] - это по сути обычный синтаксис доступа по ключу, как obj[key]
// Массивы расширяют объекты, так как предусматривают специальные методы для работы с упорядоченными коллекциями
// данных, а также свойство length. Но в основе все равно лежит объект.
// В JS существует 8 основных типов данных. Массив является объектом и ведет себя тоже как объект.
// Например копируется по ссылке. Но то, что действительно делает массивы особенными - это их внутреннее
// представляение. Движок JS старается хранить элементы массива в непрерывной области памяти,
// один за другим. Существуют и другие способы оптимизации.
// Но все они утратят эффективность, если перестать работать с массивом как упорядоченной коллекцией данных
// и начать использовать его как обычный объект.

let array = []; // создаем массив

array[99999] = 5; // создаем свойство с индексом, намного превышающим длину массива

array.test = 25; // создаем свойство с произвольным именем

// ↑↑↑ Это возможно, потому что в основе массива лежит объект. Можно присвоить ему любые свойства.
// Но движок поймет, что мы работаем с массивом, как с обычным объектом и отключит способы оптимизации,
// используемые для массивов
// 
...
// 
// Массив следует считать особой структурой. Они тщательно настроены в движках JS для работы с
// однотипными упорядоченными данным. Если нужны произвольные ключи, лучше подойдет обычный объект - {}.

// Перебор элементов:
// 
// Один из самых старых способов перебора элементов массива является цикл for по цифровым индексам
...
// Цикл for..of не представляет доступа к номеру текущего элемента, только к его значению.
// Технически, так как массив является объектом, можно использовать и вариант for..in
// Но это плохая идея, существуют скрытые недостатки способа:
    // Цикл for..in выполняет перебор всех свойств объекта, а не только цифровых. 
    // В браузере и других программных средах также существуют так называемые псевдомассивы - объекты, 
    // которые выглядят, как массив. То есть, у них есть свойство length и индексы, но они также могут
    // иметь дополнительные нечисловые свойства и методы, которые нам не нужны. Тем не менее, цикл for..in
    // выведет и их
    // 
...
// Свойство length автоматически обновляется при измеении массива, точнее, это не количество элементов массива
// а наибольший цифровой индекс плюс один
// Единственный элемент, имеющий большой индекс, дает большую длину:
...
// (массивы таким способом не используются)
// Еще один интересный факт, его можно перезаписать.
// Если вручную увеличить его, ничего непроизойдет, но если уменьшить его, массив станет короче.
...
arr.lenght = 5; // возвращаем length как было
console.log(arr5); // undefined: значения не восстановились
...
let arr6 = new Array('Apple');

// Он редко применяется, так как квадратные скобки короче. Кроме того, у него есть одна хитрая особоенность
// Если NEW ARRAY вызывается с одним аргументом, который представляет собой число, он создает массив без элементов,
// но с заданной длиной

let arr7 = new Array(2);

console.log(arr7[0]); // undefined! нет элементов

console.log(arr.length); // length 2

// Все элементы в new Array(number) равны undefined
// Чтобы избежать появления таких неожиданных ситуаций, обычно используют квадратные скобки, если, конечно, неизвестно
// по какой причине нужен именно ARRAY.

// Многомерные массивы:

// Массивы могут содержать элементы, которые тоже являются массивами. Это можно использовать для создания многомерных
// массивов, например, для хранения матриц
...
console.log(matrix[1][1]); // выведет 5, центральный элемент
...
console.log(String(arr8) === '1, 2, 3') // false быть внимательным
...
// Нельзя сравнивать массивы при помощи операторов сравнения
...
// Итак, если сравнить массивы с помощью оператора, то они никогда не будут одинаковыми, если только не сравнивать две
...
// Сравнение с примитивами также может дать 'странные' результаты:

console.log(0 == []); // true
console.log('0' == []); // false

// Здесь, в обоих случаях, сравнивается примитив с объектом массива. Таким образом, массив преобразуется в примитив
// с целью сравнения и становится пустой строкой ''. Затем продолжается процесс сравнения с примитивами:
